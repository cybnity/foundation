...
|Asset Control Domain Gateway server|Domains Gateway Server|- Java Kafka POJO (e.g Processor/Consumer as domain application service layer)<br>- Zookeeper Client<br>- Jedis<br>- Kafka Connector<br>- Spring Core<br>- SpringBoot<br>- OpenJ9 JVM<br>- Ubuntu OS<br>- Docker Image<br>- MicroK8s|Sample code which integrate the request (e.g validate a request parameter like "asset name to read") of UI layer to domain, and execute the requested feature (e.g read of an asset description) via delegation to a RT computation unit (e.g domain model of Asset Control implementing the Security Feature named Asset)|
...
|RTS Computation Unit server|Real-Time Stream Computation Unit|- Apache Flink (Asset feature java implementation)<br>- Zookeeper Client<br>- Kafka Connector<br>- Flink CEP (e.g command chain pattern implementation that build the Asset description requested by the UI layer and provider by the Domain model)<br>- OpenJ9 JVM<br>- Docker Image| |
