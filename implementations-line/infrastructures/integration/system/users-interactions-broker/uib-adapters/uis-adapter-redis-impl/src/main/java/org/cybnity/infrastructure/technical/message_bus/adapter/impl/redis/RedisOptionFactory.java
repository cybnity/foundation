...
/**
 * Factory of options reusable for Redis client instantiation.
 */
public class RedisOptionFactory {
    ...
    static public RedisOptions createUsersInteractionsSpaceOptions(String connectionUserAccount,
        ...
        StringBuilder urlBuilder = new StringBuilder().append("redis://").append(connectionUserAccount)
                .append(":").append(connectionPassword).append("@").append(serverHost).append(":").append(serverPort)
                .append("/").append(databaseNumber);
        return new RedisOptions().setConnectionString(urlBuilder.toString()).setPassword(defaultUserPassword);
    }

    /**
     * Create a default set of options allowing to discuss with Users Interactions
     * Space's write model area based on default user account and authorization password defined by the
     ...
     * @return An instance.
     * @throws IllegalArgumentException When mandatory parameter is missing (e.g any variable environment required for
     *                                  space connection).
