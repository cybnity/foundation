...
public class UserAccountRepository extends Repository {
     ...
     */
    private final ConcurrentHashMap<String, LinkedList<UserAccountDTO>> registries = new ConcurrentHashMap<String, LinkedList<UserAccountDTO>>();

    /**
     * Delegated capability regarding promotion of ReadModel changes.
     */
    private final DomainEventPublisher domainEventsManager;

    private final Logger logger = Logger.getLogger(UserAccountRepository.class.getName());

    /**
     * Write model store to monitoring regarding the updated user accounts.
     */
    private final UserAccountStore writeModelStore;
    ...
    private DomainEventSubscriber<UserAccountChanged> writeModelCreatedAccountListener = new DomainEventSubscriber<UserAccountChanged>() {
        ...
        public void handleEvent(UserAccountChanged event) {
            try {
                ...
                logger.fine("Start read model refresh about new created user account (useraccountaggregate.id="
                        + event.changedAccountRef.getEntity().identified().value().toString() + ")");
                ...
                UserAccountAggregate existInstance = writeModelStore
                        .findFrom((String) event.changedAccountRef.getEntity().identified().value());
                if (existInstance != null) {
                    append(existInstance, event.creationCommandRef);

                    // Read model is up-to-date
                    logger.fine(UserAccountRepository.class.getSimpleName()
