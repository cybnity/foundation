...
minikube start --driver=hyperkit --container-runtime=docker --profile local-env2 --nodes 2 --cpus 2 --disk-size '10g' --memory '4g' &&
...
echo "Add labels to a cluster based on 2 nodes" &&
