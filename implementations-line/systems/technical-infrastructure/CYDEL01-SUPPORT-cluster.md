...
- Create the certificate and key files ([mandatory Secret resource names](https://ranchermanager.docs.rancher.com/getting-started/installation-and-upgrade/resources/add-tls-secrets)) for K8S secret resource creation) usable by Rancher installer from the custom domain certificate and key files:
    __tls.crt__: custom domain server certificate file. It's a file (PEM formatted) including concatenated server certificate followed by any intermediate certificate(s)
...
```
    ...
    ```
- Create __cacerts.pem__ file including the custom domain CA trust chain (containing only root CA certificate or certificate chain from private CA), and create __tls-ca__ secret via command:
```
  kubectl -n cattle-system create secret generic tls-ca --from-file=cacerts.pem
```
    - Or update CA certificate secret via command:
    ```
      kubectl -n cattle-system create secret generic tls-ca --from-file=cacerts.pem \
      --dry-run=client --save-config -o yaml | kubectl apply -f -
    ```
...
```
- If it's a certificates update process performed onto an existing Rancher installation:
  - Reconfigure Rancher deployment: see the [Reconfigure Rancher deployment documentation](https://github.com/rancher/rancher-docs/blob/main/docs/getting-started/installation-and-upgrade/resources/update-rancher-certificate.md#3-reconfigure-the-rancher-deployment) to redeploy the Rancher pods (e.g. via command `kubectl rollout restart deploy/rancher -n cattle-system`)
  - Reconfigure Rancher agents: see the [Reconfigure Rancher agents documentation](https://github.com/rancher/rancher-docs/blob/main/docs/getting-started/installation-and-upgrade/resources/update-rancher-certificate.md#4-reconfigure-rancher-agents-to-trust-the-private-ca)
    - Force a redeploy of the Rancher agents for SUPPORT cluster as described into [Rancher documentation](https://github.com/rancher/rancher-docs/blob/main/docs/getting-started/installation-and-upgrade/resources/update-rancher-certificate.md#method-1-force-a-redeploy-of-the-rancher-agents) via command:
    ```
      # Reployment of SUPPORT CLUSTER ID agents
      kubectl annotate clusters.management.cattle.io local io.cattle.agent.force.deploy=true
    ```
    - Force a re-registration of the Rancher agents on each other cluster with SSH connection on each server, and execution of the agent installation command (which have been updated from Rancher UI regarding --ca-checksum value)
